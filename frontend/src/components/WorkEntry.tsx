...
export default function WorkEntry() {
  ...
  const [entryMeta, setEntryMeta] = useState<{ updatedBy: string | null } | null>(null);
  ...
  useEffect(() => {
    const fetchEntry = async () => {
      try {
        ...
        setEntryMeta({ updatedBy: data.updated_by_name ?? null });
      } catch {
        setRows([emptyRow()]);
        setEntryMeta(null);
      }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
      ...
      <div className="space-y-6">
        ...
        {/* Entry audit info */}
        {entryMeta?.updatedBy && (
          <div className="rounded-2xl bg-[#F8F6F2] px-5 py-3 text-xs text-gray-500">
            Last updated by <span className="font-medium text-[#1E1E1E]">{entryMeta.updatedBy}</span>
          </div>
        )}
